
import numpy as np
import pysoem
import ctypes
import struct
from threading import Lock,Event

from _EcatObject import EcatLogger
from _EcatSeverity import SEVERITY_CRITICAL


class SwitchController(object):

    _debug = False    
    _lock = Lock()
    _exit = Event()

    def _get_enabled(self):
        return not self._exit.is_set()
    Enabled = property(fget=_get_enabled)

    _index = 0

    _device = None
    def _get_device(self): return self._device
    Device = property(fget=_get_device)

    def _get_id(self):
        return f"{self._device.name}.{self._index}"
    Id = property(fget=_get_id)

    _deviceLock: Lock = None
    def _get_deviceLock(self): return self._deviceLock
    DeviceLock: Lock = property(fget=_get_deviceLock)

    _data = None
    def _get_data(self): return self._data
    Data = property(fget=_get_data)    

    _display = None
    def _get_display(self): 
        return self._display
    Display = property(fget=_get_display)    

    def __init__(self, index, device, lock, debug=False) -> None:   
        super().__init__()   
        self._index = index  
        self._device = device
        self._deviceLock = lock
        self._debug = debug
        
    def release(self):
        self._exit.set()

    def input(self):
        return None

    def output(self, data):        
        pass

    def write(self, data):   
        pass

    def init(self):
        return False
    
    def compute(self):
        pass
            
    def run(self):
        return False
    
    def callback(self, *args):
        pass

    def severityFunc(self, value):
        pass

    
class Beckhoff8SwitchController(SwitchController):

    def __init__(self, index, device, lock, debug=False) -> None:
        super().__init__(index, device, lock, debug)   

    _initialized = False          
    def init(self):

        rc = False
        try:
            
            if self._data is None:
                self.read()
           
        except Exception as ex:
            EcatLogger.debug(f"-- Beckhoff8SwitchController.init {ex}")
            
        finally:
            if self._data is None:
                self._data = [0] * 8
            rc = self._data is not None
            return rc

    def run(self):
        
        if not self.Enabled:
            return None

        if (self.Device.state & pysoem.OP_STATE) != self.Device.state:
            return None

        self._lock.acquire()
        try:            
            if not self._initialized:
                self._initialized = self.init()
            
            if self._initialized:
                self.compute()

        except Exception as ex:
            EcatLogger.debug(f"-- Beckhoff8SwitchController.run {ex}")

        finally:
            self._lock.release()

        return self.Data
    
    def compute(self):
        return self.read()
    
    def read(self):

        self.DeviceLock.acquire()
        try:
            a, o = 0x1600, 0x00
            self._data = [0 if ctypes.c_bool.from_buffer_copy(self.Device.sdo_read(a + i, o)).value else 1 for i in range(8)]
        except Exception as ex:
            pass
        finally:
            self.DeviceLock.release() 

    def write(self, data):
        self.DeviceLock.acquire()
        try:
            self.Device.output = data        
        except Exception as ex:
            EcatLogger.debug(f"-- Beckhoff8SwitchController.write {ex}")
        finally:
            self.DeviceLock.release() 

    def output(self, data):
        
        try:

            if self._data is None:
                return False
            self._data = data.copy()
            b = "".join([str(v) for v in data])  
            self.write(struct.pack('1B', int(b,2)))

        except Exception as ex:
            EcatLogger.error(f"-- Beckhoff8SwitchController.output {ex}")
            return False

        return True

    def callback(self, *args):

        arg = args[0]        
        name = arg["name"]
        
        if self._data is not None:
            pass

        return False
    
    def severityFunc(self, value, data=[0]*8):        
        if (np.any([v & SEVERITY_CRITICAL == SEVERITY_CRITICAL for v in value])):
            self.output(data)
