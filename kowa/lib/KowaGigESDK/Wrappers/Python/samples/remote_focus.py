...
import kge
import kge.external_control.remote_focus as remote_focus
        ...
        with camera.initFilterDriver():
            with camera.openStreamChannel():
                ...
                with camera.acquisitionStart(0):
                    ...
                    for k,v in usagetable.items():
                        print(f"    {k:8}: {v}")

                    remote_focus.setOrigin(camera)
                    frameno=-1
                    while True:
                        frameno+=1
                        try:
                            ...
                            current = remote_focus.getStepCount(camera)
                            print(f"Current step count: {current:5}", end="\r")

                            grabbed_img = np.ctypeslib.as_array(image_buffer).reshape(height,width)
                            display_img = cv2.resize(grabbed_img, dsize=dsize,
                                                     interpolation=cv2.INTER_AREA)
                            beginy = max(0, grabbed_img.shape[0] // 2)
                            beginx = max(0, grabbed_img.shape[1] // 2)
                            center_img = grabbed_img[
                                beginy:beginy + dsize[1],
                                beginx:beginx + dsize[0],
                            ]

                            display_img = cv2.cvtColor(display_img, cv2.COLOR_GRAY2BGR)
                            for i,(k,v) in enumerate(usagetable.items()):
                                cv2.putText(display_img, f"{k:8}: {v}",
                                            (0, (i+1)*20),
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,192,0),
                                )
                            cv2.putText(display_img, f"Current step count: {current:5}",
                                        (0, (len(usagetable)+1)*20),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,192,0),
                            )
                            ...
                            if key == ord("e"):
                                ...
                                remote_focus.addStepCount(camera, 1)
                            elif key == ord("s"):
                                remote_focus.addStepCount(camera, 10)
                            elif key == ord("d"):
                                remote_focus.addStepCount(camera, 100)
                            elif key == ord("z"):
                                remote_focus.subStepCount(camera, 1)
                            elif key == ord("x"):
                                remote_focus.subStepCount(camera, 10)
                            elif key == ord("c"):
                                remote_focus.subStepCount(camera, 100)
                            elif key != -1:
