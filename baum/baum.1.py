...
class baumDevice():
    ...
    def setup(self):
        ...
        try:
            ...
            self.Camera.f.SharpeningMode.value = 3

            baumLogger.debug(f"    {self.Sharpening}")
        ...
        if image is not None:
            ...
            if 'jpg' == mode:
                rc = cv2.imwrite(filename, 
                                 cv2.cvtColor(image, cv2.COLOR_BGR2RGB), [int(cv2.IMWRITE_JPEG_QUALITY), 
                                                                          IMG_QUALITY])
            else:
    ...
    def meta(self, filename:str, prefix:str, tags:dict):
        ...
        try:
            ...
            for key in tags.keys():        
                image.modify_xmp({f"Xmp.hot.{key}": tags[key] })
            sensor = self.Sensor
            for key in sensor.keys():      
                image.modify_xmp({f"Xmp.hot.sensor.{key}": sensor[key] })
        finally:            
            image.close()
        ...
        while(self.Running):            
            ...
            try:
                ...
                for device in self.Devices:
                    ...
                    if PUB_DEBUG:
                        baumLogger.debug(f"{data['id']}: {data['f']} {data['blur']}")
