...
DEV_FLIP = -1 # 0 vertical, +1 horizontal, -1 both
...
PUB_STORE_META_COL = ['T1','T2','p1','p2','source']
PUB_STORE_META_DEL = ";"
PUB_STORE_META_LF = "\n"
...
class baumDevice():
    ...
    def _set_exposureTime(self, value):
        self.Camera.f.ExposureTime.value = float(value)
    ...
    def _set_gain(self, value):
        self.Camera.f.Gain.value = float(value)
    def _get_gain(self):
    ...
    def _get_pixelFormat(self): return self.Camera.f.PixelFormat.value
    def _set_pixelFormat(self, value:int): self.Camera.f.PixelFormat.value = value
    ...
    def _get_sensor(self):
        if self._sensor is None:
            self._sensor = dict(
                ...
                shutter=self.Camera.f.SensorShutterMode.value,
                speedMode=self.Camera.f.InterfaceSpeedMode.value,
            )
        return self._sensor
                ...
                for device in self.Devices:
                    ...
                    if PUB_DEBUG:
                        if device.No in [0]:
                            ...
                            baumLogger.debug(f"{datetime.datetime.now()} {device.ExposureTime} {datetime.datetime.now()-t} ")
...
def main():
    ...
    try:
        ...
        for no in DEV_NO:
            ...
            filename = sys.argv[no+1] if len(sys.argv) > no+1 else os.path.join(os.getcwd(), f'baum.config.{no}.json')
            ...
            device.initialize(
                focusStep=device.Config.FocusStep,
                apertureStep=device.Config.ApertureStep,
                exposureTime=device.Config.ExposureTime,
                gain=device.Config.Gain,
                sharpeningMode=device.Config.SharpeningMode,
                sharpeningFactor=device.Config.SharpeningFactor,
                sharpeningThreshold=device.Config.SharpeningThreshold,
                rotation=device.Config.Rotation,
                flip=device.Config.Flip      
                )
